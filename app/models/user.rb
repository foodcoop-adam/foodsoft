...
class User < ActiveRecord::Base
  ...
  after_initialize do
    settings.defaults.merge!({
      profile: {
        language: FoodsoftConfig[:default_locale] || I18n.default_locale,
      },
      messages: {
        send_as_email: true,
      },
    ...
    if FoodsoftConfig[:user_defaults]
      settings.defaults.merge!(FoodsoftConfig[:user_defaults])
    end
  end
  ...
  def request_password_reset!
    ...
    if save!
      Mailer.reset_password(self).deliver
  ...
  def ordergroup=(attributes)
    ...
    if attributes[:id] == 'new'
      ...
      Ordergroup.build_from_user(self, attributes.reject {|i| i.to_s=='id'})
