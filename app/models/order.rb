...
class Order < ActiveRecord::Base
  ...
  def can_send
    ...
    min_order_quantity = (Float(supplier.min_order_quantity) rescue 0)
    if not supplier.min_order_quantity.blank? and min_order_quantity > 0
      min_order_quantity >= sum(:gross) or return :min_quantity
    end
    return true
  end
