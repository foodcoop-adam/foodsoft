...
class OrderArticle < ActiveRecord::Base
  ...
  def update_results!
    if order.open?
      self.quantity = collect_result(:quantity)
      self.tolerance = collect_result(:tolerance)
  ...
  def enforce_boxfill
    ...
    unless (delta_q == 0 && delta_t == 0) ||
      ...
      raise ActiveRecord::RecordNotSaved.new("Change not acceptable in boxfill phase, sorry.")
    end
  end
